...
scp svlsupport@52.177.16.236:/home/svlsupport/tree-gpc-results.csv ~/Desktop/


supp
./run_pod_products_api_latest.sh
rails c
...
ActiveRecord::Base.transaction do
  current_index = 0
  max_index =  GPC_IDS.size

  while(current_index < max_index)
    #if current_index.next % 1000 == 0
    if current_index.next % 10 == 0
      puts "
    ...
    if category
      ...
      while(current_index < end_index)
        ...
        #atributos_alado <<  current_att_id
        attribute_geted = false
        cat_prds = category.product_attributes
        cat_prds.each do |attr|
          changed = false
          changes = []
...
exit
mv product-attributes-update-result.csv /tmp/product-attributes-update-result.csv
exit
scp svlsupport@52.177.16.236:/home/svlsupport/product-attributes-update-result.csv ~/Desktop/
